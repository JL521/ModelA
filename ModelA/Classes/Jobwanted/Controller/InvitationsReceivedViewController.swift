//
//  InvitationsReceivedViewController.swift
//  Rencaiyoujia
//
...
class InvitationsReceivedViewController: JobWantBaseViewController {

    let titles = ["待接受", "已接受", "已结束"]
    var scale:CGFloat?
    lazy var childVcs:[InvitationsListViewController] = {
        var childVcs = [InvitationsListViewController]()
        
        let watvc = InvitationsListViewController()
        watvc.status = "1"
        childVcs.append(watvc)
        
        let yjs = InvitationsListViewController()
        yjs.status = "2"
        childVcs.append(yjs)
        
        let jsvc = InvitationsListViewController()
        jsvc.status = "3"
...
extension InvitationsReceivedViewController: YMPageTitleViewDelegate {
...
extension InvitationsReceivedViewController: YMPageContentViewDelegate {
